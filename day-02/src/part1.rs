...
impl<'a> Game<'a> {
    fn valid_for_gameset(&self, map: &BTreeMap<&str, u32>) -> Option<u32> {
        self.rounds
            .iter()
            .any(|round| {
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let map = BTreeMap::<&str, u32>::from([("red", 12), ("green", 13), ("blue", 14)]);
    let games = parse_games(input).expect("should parse");
    let rs = games
        .1
        .iter()
        .filter_map(|game| game.valid_for_gameset(&map))
        .sum::<u32>()
        .to_string();
    Ok(rs)
}
